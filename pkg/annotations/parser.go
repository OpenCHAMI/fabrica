...
func ParseFileAnnotations(filename string) (map[string]*ResourceAnnotations, error) {
	// Check cache first. The cache stores complete *ResourceAnnotations, so a
	// hit preserves resource-level state (IsResource, StorageMode, composite
	// indexes, migration policy) as well as fields.
