...
func (g *Generator) commonTemplateData(templateName string) map[string]interface{} {
	...
	setGeneratedFileIdentity(g.Version, g.Commit)

	return map[string]interface{}{
		"Version":       g.Version,
		"FabricaCommit": g.Commit,
...
var (
	generatedTimestampLineRE = regexp.MustCompile(`(?m)^((?://|#)\sGenerated(?: at)?:\s|Generated(?: at)?:\s).*$`)
	generatedVersionLineRE   = regexp.MustCompile(`(?mi)^// Code generated by fabrica(?: .*)?\. DO NOT EDIT\.$`)
)

var generatedIdentityLine []byte

func setGeneratedFileIdentity(version, commit string) {
	identityCommit := strings.TrimSpace(commit)
	if identityCommit == "" || identityCommit == "none" {
		generatedIdentityLine = nil
		return
	}

	identityVersion := strings.TrimSpace(version)
	if identityVersion == "" {
		identityVersion = "dev"
	}

	identity := fmt.Sprintf("%s (commit: %s)", identityVersion, identityCommit)

	generatedIdentityLine = []byte(fmt.Sprintf("// Code generated by Fabrica %s. DO NOT EDIT.", identity))
}

func applyGeneratedFileIdentity(content []byte) []byte {
	if len(generatedIdentityLine) == 0 {
		return content
	}

	if generatedVersionLineRE.Match(content) {
		return generatedVersionLineRE.ReplaceAll(content, generatedIdentityLine)
	}
...
func normalizeGeneratedTimestamps(content []byte) []byte {
	...
	return generatedVersionLineRE.ReplaceAll(normalized, []byte("// Code generated by Fabrica <normalized>. DO NOT EDIT."))
}

func writeGeneratedFile(path string, content []byte) (bool, error) {
	content = applyGeneratedFileIdentity(content)
...
func (g *Generator) ParseResourceAnnotations(filePath, resourceName string) (*annotations.ResourceAnnotations, error) {
	resourceAnnotations, err := annotations.ResolveResourceAnnotations(filePath, resourceName)
	if err != nil {
		return nil, fmt.Errorf("resolve annotations for %s: %w", resourceName, err)
	}
	...
	if err := g.validateAnnotationsForDatabase(filePath, resourceName, resourceAnnotations); err != nil {
		return nil, err
	}
...
func (g *Generator) GenerateAll() error {
	if err := g.PrepareResourceAnnotations(); err != nil {
		return err
	}
...
func (g *Generator) GenerateStorage() error {
	...
	conflictErrorsPath := filepath.Join(storageDir, "errors_generated.go")
	if err := g.executeTemplate("storageErrors", conflictErrorsPath, g.globalTemplateData("storage/errors.go.tmpl")); err != nil {
		return fmt.Errorf("failed to generate storage error contracts: %w", err)
	}
	...
	if g.StorageType == "ent" {
		resourceOpsPath := filepath.Join(storageDir, "storage_ent_resources_generated.go")
		if err := g.executeTemplate("storageEntResources", resourceOpsPath, g.globalTemplateData("storage/ent_resource_ops.go.tmpl")); err != nil {
			return fmt.Errorf("failed to generate ent resource operations: %w", err)
		}
	}
...
func (g *Generator) GenerateReconcilers() error {
	for _, resource := range g.Resources {
		...
		if _, err := writeGeneratedFile(filename, formatted); err != nil {
		...
		if _, err := os.Stat(stubFilename); os.IsNotExist(err) {
			...
			if _, err := writeGeneratedFile(stubFilename, stubFormatted); err != nil {
...
func (g *Generator) LoadTemplates() error {
	...
	templateFiles := map[string]string{
		...
		"storageErrors":       "storage/errors.go.tmpl",
		"storageEnt":          "storage/ent.go.tmpl",
		"storageEntResources": "storage/ent_resource_ops.go.tmpl",
...
func (g *Generator) generateMiddlewareFile(templateName, filename, outputDir string, data interface{}) error {
	...
	written, err := writeGeneratedFile(fullPath, formatted)
	if err != nil {
...
func (g *Generator) GenerateOpenAPI() error {
	...
	if _, err := os.Stat(extStubFilename); os.IsNotExist(err) {
		...
		if _, err := writeGeneratedFile(extStubFilename, extFormatted); err != nil {
...
func (g *Generator) GenerateEntAdapter() error {
	...
	for _, resource := range g.Resources {
		...
		stalePath := filepath.Join(storageDir, "ent_adapter_"+strings.ToLower(resource.Name)+".go")
		if err := os.Remove(stalePath); err != nil && !os.IsNotExist(err) {
			return fmt.Errorf("remove stale dedicated adapter for %s: %w", resource.Name, err)
		}
	}
...
func (g *Generator) generateDedicatedAdapter(resource ResourceMetadata) error {
	...
	schemaData, err := g.dedicatedSchemaData(resource)
	if err != nil {
		return fmt.Errorf("build adapter field mappings: %w", err)
	}
	data["DedicatedFields"] = schemaData.Fields
...
func (g *Generator) executeTemplate(templateName, outputPath string, data interface{}) error {
	...
	written, err := writeGeneratedFile(outputPath, output)
	if err != nil {
